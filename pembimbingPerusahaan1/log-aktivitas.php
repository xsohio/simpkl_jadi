...
// ── Insert log: buka halaman ini = aktivitas "Melihat log aktivitas" ───────
require_once 'log_helper.php';
logAktivitas($pdo, $pid, 'Melihat halaman log aktivitas', 'pembimbing');

// ── Filter ────────────────────────────────────────────────────────────────
$filter_jenis = $_GET['jenis'] ?? 'semua'; // semua | login | jurnal | laporan | nilai | bimbingan | lainnya
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_log'])) {
    ...
    if ($aktivitas_baru !== '') {
        require_once 'log_helper.php';
        logAktivitas($pdo, $pid, $aktivitas_baru, 'pembimbing');
    }
...
function getLogStyle(string $aktivitas): array {
    ...
    if (str_contains($a,'bimbingan')) return ['#818cf8','rgba(99,102,241,.12)','fa-comments'];
    if (str_contains($a,'absensi'))   return ['#2dd4bf','rgba(20,184,166,.12)','fa-calendar-check'];
    if (str_contains($a,'kompetensi'))return ['#c084fc','rgba(168,85,247,.12)','fa-tasks'];
...
<div class="panel" style="margin-bottom:20px;padding:18px 20px">
  ...
  <form method="POST" action="" style="display:flex;gap:10px;align-items:flex-end">
    <div style="flex:1">
      ...
      <input type="text" name="aktivitas_manual" required
             placeholder="Contoh: Melakukan kunjungan ke tempat PKL siswa…"
  ...
  <p style="margin-top:8px;font-size:.72rem;color:#475569">
    <i class="fas fa-info-circle" style="margin-right:4px"></i>
    Gunakan fitur ini untuk mencatat aktivitas manual seperti kunjungan lapangan, rapat, atau kegiatan bimbingan di luar sistem.
  </p>
</div>
...
<div class="panel" style="margin-bottom:20px;padding:18px 20px">
  <form method="GET" action="">
    ...
    <div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap">
      <?php $jenis_list = ['semua'=>'Semua','login'=>'Login/Logout','jurnal'=>'Jurnal','laporan'=>'Laporan','nilai'=>'Nilai','bimbingan'=>'Bimbingan']; ?>
  ...
  <div style="display:flex;flex-direction:column;gap:0">
    ...
    <div class="activity-item" style="padding:14px 4px;display:flex;align-items:flex-start;gap:14px;border-bottom:1px solid rgba(255,255,255,.04)">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-size:.875rem;color:#e2e8f0;font-weight:500;line-height:1.4">
...
<div style="margin-top:16px;padding:14px 16px;background:rgba(99,102,241,.07);border:1px solid rgba(99,102,241,.15);border-radius:10px;font-size:.78rem;color:#64748b">
  ...
  Log aktivitas ini hanya menampilkan aktivitas akun Anda. Admin dapat melihat seluruh log dari semua pengguna di panel admin.
</div>
